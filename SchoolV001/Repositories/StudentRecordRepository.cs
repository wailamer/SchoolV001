...
public class StudentRecordRepository
{
    ...
    {
        var doc = _storage.LoadOrCreate(FileName, "StudentRecords");
    ...
    public StudentRecord? LastForStudent(int studentId) => GetAll().Where(x => x.StudentId == studentId).OrderByDescending(x => x.Id).FirstOrDefault();
    ...
    public void Add(StudentRecord record)
    {
        ...
        doc.Root?.Add(new XElement("Record",
            ...
            new XElement("EnrollmentDate", DateTime.UtcNow.ToString("yyyy-MM-dd")),
